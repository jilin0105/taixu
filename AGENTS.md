...
- **核心定位**：在 **Android 无 Root 用户态** 下，基于 **PRoot** 运行完整的 **Debian Linux 沙箱**，并深度集成 **AI Agent Harness 智能体引擎**、**原生 PTY 终端**、**工作区文件管理** 与 **Material 3 Expressive UI**。
- **Android / UI 层**：
  - **语言**：Kotlin 2.0+ (100% Kotlin + Jetpack Compose)
...
- **底层运行时 (Runtime & Native)**：
  - **C/C++ JNI**：`src/main/cpp/pty.c` (openpty, fork, exec, ioctl 终端 PTY 桥接，编译为 `libtaixu_pty.so`, `arm64-v8a`)
  - **Linux 容器**：内置 PRoot 引擎，启动 OCI Debian 13-slim rootfs
...
- **AI 智能体引擎 (Agent Harness)**：
  ...
  - **本地端侧模型**：沙箱内原生支持 `llama.cpp` (`http://127.0.0.1:8080/v1`) 与 `Ollama` (`http://127.0.0.1:11434/v1`)，无需 API Key
...
├── runtime/              # Linux 沙箱与 PRoot 核心：ProotCommandBuilder, PTY 会话, WorkspaceManager
├── harness/              # Agent 智能体核心：ReAct 循环、流式推理、工具调度、上下文裁剪
├── tools/                # 工具生态中心：agent_providers.json、ProviderCatalog、安装器适配
└── feature/              # Compose UI 业务特性层
    ...
    ├── terminal/        # 真实 Linux 控制台、VT100 终端渲染、触觉按键条 (Haptics)
    ├── workspace/       # 工作区管理器、项目代码浏览器、/sdcard 宿主直通
    ├── settings/        # 设置中心：模型档案管理、存储挂载管理、外观与诊断
...
| 业务领域 | 关键文件路径 | 核心职责 |
| :--- | :--- | :--- |
| **Agent 核心** | [`harness/.../HarnessLoop.kt`](harness/src/main/java/top/wkbin/taixu/harness/HarnessLoop.kt) | ReAct 思考与工具调用闭环调度 |
| **模型 Provider** | [`tools/.../assets/agent_providers.json`](tools/src/main/assets/agent_providers.json) | OpenAI / DeepSeek / 本地 llama.cpp 预设配置 |
| **模型目录管理** | [`tools/.../AgentProviderCatalog.kt`](tools/src/main/java/top/wkbin/taixu/core/tools/AgentProviderCatalog.kt) | Provider 读取、校验与安全策略 |
...
| **工作区与沙箱文件** | [`runtime/.../WorkspaceManager.kt`](runtime/src/main/java/top/wkbin/taixu/runtime/WorkspaceManager.kt) | 项目目录解析、`/sdcard` 映射与沙箱路径隔离 |
...
| **存储挂载管理** | [`feature/settings/.../StorageMountSettingsScreen.kt`](feature/settings/src/main/java/top/wkbin/taixu/ui/settings/StorageMountSettingsScreen.kt) | 宿主存储与 Download 挂载开关 UI |
...
| **Native PTY C** | [`app/src/main/cpp/pty.c`](app/src/main/cpp/pty.c) | JNI PTY 核心底层实现 |
