            ...
            components = listOf(
                PluginComponent(
                    ...
                    postInstallSteps = listOf(
                        "mkdir -p /opt /usr/local/bin \$TAIXU_TOOL_DIR/bin 2>/dev/null || true",
                    ...
                    checkCommand = "(command -v aapt || test -x /usr/bin/aapt) && (command -v java || test -x /usr/lib/jvm/java-17-openjdk-arm64/bin/java)",
                ),
                PluginComponent(
                    id = "flutter",
