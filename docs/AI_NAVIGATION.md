...
├── runtime/              # Linux 沙箱与 PRoot 核心：ProotCommandBuilder, PTY 会话, 工作区
...
- **依赖方向**：`feature/*` → `core/*` / `runtime` / `harness` / `tools`，`feature` 模块之间**禁止互相依赖**（共享 UI 走 `feature/components` 与 `feature/theme`）。
