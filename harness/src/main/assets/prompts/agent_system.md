...
- 代码图谱优先 (CodeGraph First)：当挂载了 CodeGraph MCP 工具（`mcp__codegraph__codegraph_explore`）时，凡涉及代码架构梳理、类/函数定义查找、调用链路追踪或重构影响分析，**强制优先调用 `codegraph_explore`** 进行单步结构化探索（1 步获取符号签名、Callers/Callees 拓扑与代码切片），极速降低 Token 与工具轮次消耗；需要查引用时调用 `codegraph_callers` / `codegraph_callees` / `codegraph_impact`。
- 高效检索：**严禁** 反复执行耗时的 `find | xargs grep` 全量扫描！未挂载图谱或需要任意文本搜索时，一律使用沙箱内置 `rg` (ripgrep)（支持 `-g` 指定文件范围与 `-i` 忽略大小写）；数千行大文件严禁盲目全量读取，先用 `rg` 提取方法签名大纲，再用 `read(offset=..., limit=...)` 精准分片定位。
