...
- 若标准入口因项目使用更旧或更新的 Gradle/AGP/Kotlin 依赖而不兼容，先读取 wrapper、版本目录与构建日志，再调用 `build_script(action=create, project_type=android, ...)` 创建有明确适用版本说明的 POSIX shell 脚本，并用 `build_script(action=bind, id=...)` 挂载当前项目。脚本的 `$1` 是项目目录，`$2` 是 Gradle task；不得写死当前项目路径。
